...
export default {
  ...
  adminLookupAPIKeys: (ids) => Get('/admin/api-keys/lookup', { params: { ids: (ids || []).join(',') } }),
  ...
  // Admin - Account Health Check (账号健康检测)
  getHealthCheckStatus: () => Get('/admin/health-check/status'),
  triggerHealthCheck: () => Post('/admin/health-check/trigger'),
  checkAccountHealth: (accountId) => Post(`/admin/accounts/${accountId}/health-check`),
  ...
  // Admin - Error Rules (错误规则配置)
  getErrorRules: (params) => Get('/admin/error-rules', { params }),
  getErrorRule: (id) => Get(`/admin/error-rules/${id}`),
  createErrorRule: (data) => Post('/admin/error-rules', data),
  updateErrorRule: (id, data) => Put(`/admin/error-rules/${id}`, data),
  deleteErrorRule: (id) => Delete(`/admin/error-rules/${id}`),
  resetErrorRules: () => Post('/admin/error-rules/reset'),
  refreshErrorRulesCache: () => Post('/admin/error-rules/refresh'),
  enableAllErrorRules: () => Put('/admin/error-rules/enable-all'),
  disableAllErrorRules: () => Put('/admin/error-rules/disable-all'),

  // Admin - Model Mappings (模型映射)
  getModelMappings: () => Get('/admin/model-mappings'),
  getModelMapping: (id) => Get(`/admin/model-mappings/${id}`),
  createModelMapping: (data) => Post('/admin/model-mappings', data),
  updateModelMapping: (id, data) => Put(`/admin/model-mappings/${id}`, data),
  deleteModelMapping: (id) => Delete(`/admin/model-mappings/${id}`),
  toggleModelMapping: (id) => Post(`/admin/model-mappings/${id}/toggle`),
  refreshModelMappingCache: () => Post('/admin/model-mappings/refresh'),
  getModelMappingCacheStats: () => Get('/admin/model-mappings/cache')
}
